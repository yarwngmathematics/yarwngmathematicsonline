...
// ── CONFIG — keep in sync with page.tsx ──────────────────────────────────────
const SCRIPT_URL =
...
function PaymentSuccessInner() {
  ...
  const [state, setState] = useState<
    "loading" | "verifying" | "success" | "failed" | "error"
  >("loading");
  ...
  useEffect(() => {
    ...
    const verify = async () => {
      try {
        ...
        // If verification API fails but PhonePe already returned status=success,
        // trust it rather than blocking the student.
        if (status === "success") {
          setState("success");
        } else {
          setState("failed");
  ...
  useEffect(() => {
    ...
    const submitSheet = async () => {
      try {
        ...
        // Google Apps Script JSONP-style — use no-cors because GAS doesn't
        // return CORS headers on the first redirect.
